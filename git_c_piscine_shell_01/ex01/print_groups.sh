...
# Firstly, we retreive the information about the user for $FT_UVER environmental variable. To get the name-only version for all the groups we use the flag -nG.
# Then we pipe that information to sed to remplace (substitute, thatś why we use ´s/) all the spaces with comma characters.
